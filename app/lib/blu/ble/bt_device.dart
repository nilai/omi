...
class BtDevice {
  ...
  static bool isAiNoteDevice(ScanResult result) {
    ...
    // 第一优先级: 检查广播数据中的 Service UUID (协议定义，最可靠)
    ...
    if (hasServiceUuid) {
      debugPrint('[BtDevice] ✓ 通过 Service UUID 识别为 Note 设备: mac=${result.device.remoteId}');
      return true;
    }

    // 第二优先级: 检查设备名称前缀
    // 支持多种命名规范: AI_PIN, AI_CARD, AI_PEN, AI_NOTE, NOTE, TX_NOTE
    final isNoteByName = name.startsWith('AI_PIN') ||   // Ai_Pin 设备
        name.startsWith('AI_CARD') ||  // AI_CARD 协议规范
        name.startsWith('AI_PEN') ||
        name.startsWith('AI_NOTE') ||
        name.startsWith('AI_MIC') ||   // AI_MIC 设备
        name.startsWith('NOTE') ||
        name.startsWith('TX_NOTE');

    if (isNoteByName) {
      debugPrint('[BtDevice] ✓ 通过名称识别为 Note 设备: name="$name", mac=${result.device.remoteId}');
      return true;
    }
  ...
  static bool isAiNoteDeviceFromDevice(BluetoothDevice device) {
    ...
    // 第一优先级: Service UUID
    final hasServiceUuid = device.servicesList
        .any((s) => s.uuid.toString().toLowerCase() == aiNoteServiceUuid.toLowerCase());
