...
class _MPFoundDevicesState extends State<MPFoundDevices> {
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          _buildClearDeviceCard(context, provider),
          const SizedBox(height: 24),
        ],
      ),
    );
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _buildBleDebugCard(context),
          const SizedBox(height: 16),
          _buildClearDeviceCard(context, finderProvider),
          const SizedBox(height: 24),
        ],
      ),
    );
  ...
  /// 构建BLE调试入口卡片
  Widget _buildBleDebugCard(BuildContext context) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 20),
      decoration: BoxDecoration(
        color: Colors.white,
  ...
  /// 构建清除设备重新扫描卡片
  Widget _buildClearDeviceCard(BuildContext context, MPDeviceFinderProvider finderProvider) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 20),
      decoration: BoxDecoration(
        color: Colors.white,
        ...
        onTap: () async {
          ...
          final confirmed = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              title: const Text('清除设备绑定'),
              content: const Text('这将断开当前设备连接并清除保存的设备信息。\n\n如果您的设备MAC地址已更换，请使用此功能重新扫描连接新设备。'),
              actions: [
                TextButton(
          ...
          if (confirmed == true) {
            await finderProvider.clearDeviceAndRescan();
          }
        },
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
