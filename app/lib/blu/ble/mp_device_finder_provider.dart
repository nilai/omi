...
class MPDeviceFinderProvider extends BaseProvider implements IDeviceServiceSubsciption {
  ...
  /// 清除设备绑定并重新扫描
  /// 用于当保存的设备MAC地址不正确时，允许用户重新选择设备
  Future<void> clearDeviceAndRescan() async {
    debugPrint('[MPDeviceFinderProvider] 清除设备绑定并重新扫描');

    // 1. 停止当前扫描
    stopScanning();

    // 2. 断开当前设备连接
    if (_deviceProvider != null && _deviceProvider!.connectedDevice != null) {
      final deviceId = _deviceProvider!.connectedDevice!.id;
      final connection = await ServiceManager.instance().device.ensureConnection(deviceId);
      if (connection != null) {
        await connection.disconnect();
      }
      // 更新 DeviceProvider 状态
      _deviceProvider!.setConnectedDevice(null);
      _deviceProvider!.setIsConnected(false);
    }

    // 3. 清除 SharedPreferences 中的设备信息
    SharedPreferencesUtil().btDeviceSet(BtDevice(id: '', name: '', type: DeviceType.aiNote, rssi: 0));
    SharedPreferencesUtil().deviceName = '';

    // 4. 清除 NoteStorageManager 中的绑定信息
    try {
      await NoteStorageManager().clearDeviceBinding();
    } catch (e) {
      debugPrint('[MPDeviceFinderProvider] 清除 NoteStorage 绑定失败: $e');
    }
    ...
    // 6. 重新开始扫描
    await startScanAndAutoConnect();

    debugPrint('[MPDeviceFinderProvider] 设备绑定已清除，开始重新扫描');
  }
